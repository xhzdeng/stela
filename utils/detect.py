...
def im_detect(model, src, target_sizes, use_gpu=True):
    if isinstance(target_sizes, int):
        target_sizes = [target_sizes]
    if len(target_sizes) == 1:
        return single_scale_detect(model, src, target_size=target_sizes[0], use_gpu=use_gpu)
    else:
        ms_dets = None
        for ind, scale in enumerate(target_sizes):
            cls_dets = single_scale_detect(model, src, target_size=scale, use_gpu=use_gpu)
...
def single_scale_detect(model, src, target_size, use_gpu=True):
    im, im_scales = Rescale(target_size=target_size, keep_ratio=False)(src)
